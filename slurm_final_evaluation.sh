...
#SBATCH --output=/home/x_anbue/finellama-rl/logs/final_eval_%j.out
#SBATCH --error=/home/x_anbue/finellama-rl/logs/final_eval_%j.err
...
echo "Starting final evaluation using fixed script..."
echo "Using model: $MODEL_ID"
echo "GSM8K data: $GSM8K_DATA_PATH"
echo "Number of examples: $NUM_EXAMPLES"
echo "Results will be saved to: $OUTPUT_DIR"

# Run the evaluation using our fixed script
apptainer exec --nv \
  --env PYTHONPATH="${PROJECT_ROOT}:$PYTHONPATH" \
  ${APPTAINER_ENV} \
  python ${PROJECT_ROOT}/fixed_eval.py \
  --model_id ${MODEL_ID} \
